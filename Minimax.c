...
int piece_score(char piece, int turn){
	...
	int sign=((p==turn)?1:-1);
	piece=tolower(piece);
	int pawn=1;	int knight=3;int bishop=3;int rook=5;int queen=9;int king=400;
	switch(piece){
	case 'm':
		return pawn*sign;
	case 'n':
...
int alphabeta(Config c, List* moves){
	...
	if(rec_alphabeta(c, moves,c.DEPTH, -100000, 100000, 1, c.TURN,&board_num)==20000){
		return 0;
	}
...
//		printf("\n--------------------------------------------------------------\n");
	...
	if (max_player) {
		...
		while (i < moves->size) {
			...
			Config new= create_new_config(&c, 1-c.TURN, child.board, c.DEPTH-1);
			int new_score = rec_alphabeta(new, next_moves, depth, alpha,beta, 0, score_turn,board_num);
		...
		while (i < moves->size) {
			...
			Config new= create_new_config(&c, 1-c.TURN, child.board, c.DEPTH-1);
			int new_score = rec_alphabeta(new, next_moves, depth, alpha,beta, 1, score_turn,board_num);
			v = min(v,new_score);
			beta = min(beta, v);
			if (beta < alpha) {
				break;
